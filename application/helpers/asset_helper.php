...
function other_asset_url($asset_name, $module_name = NULL, $asset_type = NULL)
{
 ...
 function opsi($st){
		if($st == 0){
			$urlView=base_url()."customer/".$st;
			$res="<a class='btn btn-info btn-mini' href='".$urlView."'>View $st</a>
				<a class='btn btn-danger btn-mini' href='#'>Delete</a>
				<a class='btn btn-success btn-mini' href='#'>Posting</a>";
		}elseif($st == 1){
			$res="<a class='btn btn-info btn-mini' href='#'>View</a>
				<a class='btn btn-success btn-mini' href='#'>Posting</a>";
		}else{
			$res=$st;
		}
