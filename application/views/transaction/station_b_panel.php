...
<div class="page-content" style="margin-left: 0px;">
    <div class="container-fluid">
        <div class="row-fluid">
            <div class="span12">
                <p style="font-size: 24px; margin-top: 10px;">MODUL: <?php echo $module ?></p>
            </div>
        </div>
        <div class="row-fluid">
            <?php
                foreach($panel as $row){
                    $url = "transaction/station_b_qc/".$production_order."/".$batch."/".$type."/".$row['production_order_item_id'];

                    // if($row['total_scan'] == $row['quantity'] && $row['status'] == "1")
                    //     $class = "item-complete";
                    // elseif($row['total_scan'] < $row['quantity'] && ($row['status'] == "0" || $row['status'] == "1"))
                    //     $class = "item-pending-approval";
                    ...
                    if($row['total_scan'] == $row['quantity'])
                        $class = "item-complete";
                    elseif($row['qty_remaining'] > 0 && $row['qc_scan'] == "1")
                        $class = "item-pending-approval";
                    elseif($row['qty_remaining'] > 0 && $row['qc_scan'] == "0")
                        $class = "item-not-complete";
                    elseif($row['flag_reject_qc'] == "1")
                        $class = "item-reject";
                    else
                    ...
                    <a href="<?php echo site_url($url); ?>" target="_blank" style="text-decoration: none;">
                        <div class="span2" style="margin-left: 15px; margin-bottom: 25px;">
                            <table class="<?php echo $class ?>" style="margin: 0 auto; color: #000000;">
                                <tr>
                                    <td colspan="2" class="border-1 border-2 border-3" style="text-align: center; width: 200px;">PANEL: <?php echo $row['component_no'] ?></td>
                                </tr>
                                <tr>
                                    <td class="border-3 border-2" style="text-align: center; width: 100px;"><?php echo $row['total_scan'] ?></td>
                                    <td class="border-3" style="text-align: center; width: 100px;"><?php echo $row['quantity'] ?></td>
                                </tr>
                            </table>
                        </div>
                    </a>
